...
from it.ds4biz.feature.FeatureUtils import getBaseWordFeatures,\
    getBaseTargetWordFeatures, getBaseTargetWordNumberFeatures
from it.ds4biz.obj.FeatureDictionary import FeatureDictionary
...
class WindowsTextFeatureExtractor(TextFeatureExtractor):
    ...
    def __init__(self,pre_size=2,post_size=2, replace_num = None):
        self.replace_num = replace_num
        self.pre_size=pre_size
        self.post_size=post_size
        
        
    def extract(self, tokens,replace_num=False):
        
        #hot fix da correggere e riallenare i modelli
        self.replace_num = replace_num
        ...
        for word_target,pox,pre_words,post_words in context(tokens,self.pre_size, self.post_size):
            ...
            feat_dict = getBaseTargetWordNumberFeatures(word_target,feat_dict,"target")
            feat_dict=getBaseWordFeatures(word_target,feat_dict,'target')
            
            for ind, word in enumerate(pre_words):
                if self.replace_num and word.isnumeric():
                    feat_dict["pred_word_" + str(ind)+"_t"] = "###"
                else: 
            ...
            for ind, word in enumerate(post_words):
                if self.replace_num and word.isnumeric():
                    feat_dict["foll_word_" + str(ind)+"_t"] = "###"
                else: 
